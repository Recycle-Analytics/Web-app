    ...
    leerConocidos(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (var i = 0; i < parseInt(actByScroll); i++) {
                ...
                var oldRecolectorContenedor = yield database_1.default.query('SELECT * FROM contenedores WHERE actualizacion=(GREATEST(IF((SELECT max(actualizacion) FROM contenedores) IS NULL,0,(SELECT max(actualizacion) FROM contenedores)),IF((SELECT max(actualizacion) FROM camiones) IS NULL,0,(SELECT max(actualizacion) FROM camiones)))-?-?*(?-1)-?) ORDER BY ID desc;', [i, actByScroll, numScroll, actRequested]);
    ...
    takeAct(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (type == "Contenedores") {
                const newRecolectorContenedor = yield database_1.default.query('SELECT * FROM contenedores WHERE actualizacion=(SELECT max(actualizacion) FROM contenedores) ORDER BY Fecha desc;');
