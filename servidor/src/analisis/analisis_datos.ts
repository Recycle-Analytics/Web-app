...
class AnalisisDatos {
	...
	public async actualizarEstadistica(stat: String): Promise<void>{
		...
		switch (stat){
			//General
			case "Pc_T_Ocupado": 
				await pool.query('UPDATE estadisticas SET dato=ROUND((SELECT SUM(t.PESO)*100/(COUNT(t.id_obj)*?) FROM (SELECT id_obj, MAX(id) as ID FROM test GROUP BY id_obj) r INNER JOIN test t ON t.id_obj = r.id_obj AND t.id = r.ID),1) WHERE nombre=?;', [analisisDatos.capMaxCont, stat]);
				break;
			case "Tn_T_Recogidas": 
			...
			case "Ts_N_Recoleccion": // OJO, Dependencia de la propia tabla de estadisticas
				await pool.query('UPDATE estadisticas SET dato=ROUND((((SELECT SUM(t.PESO)/1000 FROM (SELECT id_obj, MAX(id) as ID FROM test GROUP BY id_obj) r INNER JOIN test t ON t.id_obj = r.id_obj AND t.id = r.ID)-(SELECT dato FROM estadisticas WHERE nombre="Tn_T_Ocupadas"))/?),1) WHERE nombre=?', [analisisDatos.dTime, stat]);
				break;
			...
			case "Ds_Contenedores": 
				await pool.query('UPDATE estadisticas SET dato=ROUND(((SELECT COUNT(DISTINCT(id_obj)) FROM test)/?),1) WHERE nombre=?', [analisisDatos.m2Ciudad, stat]);
				break;
			case "Tn_T_Ocupadas": 
				await pool.query('UPDATE estadisticas SET dato=ROUND((SELECT SUM(t.PESO)/1000 FROM (SELECT id_obj, MAX(id) as ID FROM test GROUP BY id_obj) r INNER JOIN test t ON t.id_obj = r.id_obj AND t.id = r.ID),1) WHERE nombre=?', [stat]);
				break;
			//Contenedores
			case "Cnt_T_Funcionando": 
				await pool.query('UPDATE estadisticas SET dato=ROUND((SELECT COUNT(DISTINCT(id_obj)) FROM test),1) WHERE nombre=?', [stat]);
				break;
			case "Pc_Cnt_Llenos": 
				await pool.query('UPDATE estadisticas SET dato=ROUND(((SELECT COUNT(t.id_obj) FROM (SELECT id_obj, MAX(id) as ID FROM test GROUP BY id_obj) r INNER JOIN test t ON t.id_obj = r.id_obj AND t.id = r.ID WHERE t.PESO>?)*100/(SELECT COUNT(DISTINCT(id_obj)) FROM test)),1) WHERE nombre=?', [analisisDatos.margFullCont, stat]);
				break;
			//Vehiculos
			case "Vh_T_Disponibles": 
				...
				break;
			case "Vh_T_Operando": 
				await pool.query('UPDATE estadisticas SET dato=ROUND((SELECT COUNT(DISTINCT(IdVehiculo)) FROM camiones WHERE Encendido=1),1) WHERE nombre=?', [stat]);
				break;
			//Rutas
